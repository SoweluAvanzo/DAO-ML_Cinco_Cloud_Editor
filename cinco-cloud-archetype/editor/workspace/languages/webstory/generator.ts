...
import { GeneratorAction, Action, hasArrayProp, AnyObject, hasStringProp, hasBooleanProp } from '@cinco-glsp/cinco-glsp-common';
...
export class WebstoryGenerator extends GeneratorHandler {
    ...
    generate(model: ModelElement): void {
        ...
        const firstActivity = startMarker?.outgoingEdges[0].index.findNode(startMarker?.outgoingEdges[0].targetID);
        if (!firstActivity) {
    ...
    getContent(activities: Activity[], variables: Variable[]): string {
        ...
        const conditionsHtml: string[] = conditions.map(c => `
        ...
        `);
        const modifyVariablesHtml: string[] = modifyVariables.map(m => `
