...
@Entity(name="entity_primerefs_primectonode")
public class PrimeCToNodeDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.primerefs.SourceNodeDB pr_SourceNode;
	...
	public PanacheEntity getPr() {
		if(pr_SourceNode != null) {
			return pr_SourceNode;
		}
		return null;
	}
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_SourceNode != null && !pr_SourceNode.equals(e)) {
				pr_SourceNode.delete();
			}
		}
		...
		if(e instanceof entity.primerefs.SourceNodeDB) {
			// null all other types
			// set element
			pr_SourceNode = (entity.primerefs.SourceNodeDB) e;
			return;
		}
