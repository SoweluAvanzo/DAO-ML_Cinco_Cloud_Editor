...
@Entity(name="entity_primerefs_primectographmodel")
public class PrimeCToGraphModelDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.primerefs.PrimeRefsDB pr_PrimeRefs;
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_PrimeRefs != null && !pr_PrimeRefs.equals(e)) {
				pr_PrimeRefs.delete();
			}
		}
		...
		if(e instanceof entity.primerefs.PrimeRefsDB) {
			// null all other types
			// set element
			pr_PrimeRefs = (entity.primerefs.PrimeRefsDB) e;
			return;
		}
