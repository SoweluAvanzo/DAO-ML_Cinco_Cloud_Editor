...
@Entity(name="entity_primerefs_primectographmodelflow")
public class PrimeCToGraphModelFlowDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.flowgraph.FlowGraphDB pr_FlowGraph;
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_FlowGraph != null && !pr_FlowGraph.equals(e)) {
				pr_FlowGraph.delete();
			}
		}
		...
		if(e instanceof entity.flowgraph.FlowGraphDB) {
			// null all other types
			// set element
			pr_FlowGraph = (entity.flowgraph.FlowGraphDB) e;
			return;
		}
