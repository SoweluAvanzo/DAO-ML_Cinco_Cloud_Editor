...
@Entity(name="entity_primerefs_primectocontainerflow")
public class PrimeCToContainerFlowDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.flowgraph.SwimlaneDB pr_Swimlane;
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_Swimlane != null && !pr_Swimlane.equals(e)) {
				pr_Swimlane.delete();
			}
		}
		...
		if(e instanceof entity.flowgraph.SwimlaneDB) {
			// null all other types
			// set element
			pr_Swimlane = (entity.flowgraph.SwimlaneDB) e;
			return;
		}
