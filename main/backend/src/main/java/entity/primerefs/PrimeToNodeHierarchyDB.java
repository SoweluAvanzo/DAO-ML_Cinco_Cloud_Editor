...
@Entity(name="entity_primerefs_primetonodehierarchy")
public class PrimeToNodeHierarchyDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.hierarchy.DDB pr_D;
	
	@javax.persistence.ManyToOne
	public entity.hierarchy.ADB pr_A;
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_D != null && !pr_D.equals(e)) {
				pr_D.delete();
			}
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_A != null && !pr_A.equals(e)) {
				pr_A.delete();
			}
		}
		...
		if(e instanceof entity.hierarchy.DDB) {
			...
			pr_A = null;
			// set element
			pr_D = (entity.hierarchy.DDB) e;
			return;
		} else if(e instanceof entity.hierarchy.ADB) {
			// null all other types
			pr_D = null;
			// set element
			pr_A = (entity.hierarchy.ADB) e;
			return;
		}
