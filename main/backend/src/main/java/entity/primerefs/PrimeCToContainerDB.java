...
@Entity(name="entity_primerefs_primectocontainer")
public class PrimeCToContainerDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.primerefs.SourceContainerDB pr_SourceContainer;
	...
	public PanacheEntity getPr() {
		if(pr_SourceContainer != null) {
			return pr_SourceContainer;
		}
		return null;
	}
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_SourceContainer != null && !pr_SourceContainer.equals(e)) {
				pr_SourceContainer.delete();
			}
		}
		...
		if(e instanceof entity.primerefs.SourceContainerDB) {
			// null all other types
			// set element
			pr_SourceContainer = (entity.primerefs.SourceContainerDB) e;
			return;
		}
		
		// default-case
		// null all types
		pr_SourceContainer = null;
	}
