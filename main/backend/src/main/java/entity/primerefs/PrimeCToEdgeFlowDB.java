...
@Entity(name="entity_primerefs_primectoedgeflow")
public class PrimeCToEdgeFlowDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.flowgraph.LabeledTransitionDB pr_LabeledTransition;
	...
	public PanacheEntity getPr() {
		if(pr_LabeledTransition != null) {
			return pr_LabeledTransition;
		}
		return null;
	}
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_LabeledTransition != null && !pr_LabeledTransition.equals(e)) {
				pr_LabeledTransition.delete();
			}
		}
		...
		if(e instanceof entity.flowgraph.LabeledTransitionDB) {
			// null all other types
			// set element
			pr_LabeledTransition = (entity.flowgraph.LabeledTransitionDB) e;
			return;
		}
		
		// default-case
		// null all types
		pr_LabeledTransition = null;
	}
