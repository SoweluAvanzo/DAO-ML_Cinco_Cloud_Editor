...
@Entity(name="entity_primerefs_primectoedge")
public class PrimeCToEdgeDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.primerefs.SourceEdgeDB pr_SourceEdge;
	...
	public PanacheEntity getPr() {
		if(pr_SourceEdge != null) {
			return pr_SourceEdge;
		}
		return null;
	}
	...
	public void setPr(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(pr_SourceEdge != null && !pr_SourceEdge.equals(e)) {
				pr_SourceEdge.delete();
			}
		}
		...
		if(e instanceof entity.primerefs.SourceEdgeDB) {
			// null all other types
			// set element
			pr_SourceEdge = (entity.primerefs.SourceEdgeDB) e;
			return;
		}
