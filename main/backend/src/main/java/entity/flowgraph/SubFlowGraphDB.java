...
public class SubFlowGraphDB extends PanacheEntity {
	...
	@javax.persistence.ManyToOne
	public entity.flowgraph.FlowGraphDB subFlowGraph_FlowGraph;
	...
	public PanacheEntity getSubFlowGraph() {
		if(subFlowGraph_FlowGraph != null) {
			return subFlowGraph_FlowGraph;
		}
		return null;
	}
	...
	public void setSubFlowGraph(PanacheEntity e, boolean deleteOld) {
		...
		if(deleteOld) {
			// if no element to delete or the element to delete is
			// same that will be set then dont delete
			if(subFlowGraph_FlowGraph != null && !subFlowGraph_FlowGraph.equals(e)) {
				subFlowGraph_FlowGraph.delete();
			}
		}
		...
		if(e instanceof entity.flowgraph.FlowGraphDB) {
			// null all other types
			// set element
			subFlowGraph_FlowGraph = (entity.flowgraph.FlowGraphDB) e;
			return;
		}
		
		// default-case
		// null all types
		subFlowGraph_FlowGraph = null;
	}
