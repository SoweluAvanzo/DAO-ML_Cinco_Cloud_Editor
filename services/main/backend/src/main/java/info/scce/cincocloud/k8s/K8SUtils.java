...
public class K8SUtils {
    ...
    public static Optional<PersistentVolumeClaim> getPersistentVolumeClaimByName(KubernetesClient client, String name) {
        return client.persistentVolumeClaims().list().getItems().stream()
                .filter(s -> s.getMetadata().getName().equals(name))
                .findFirst();
    }

    public static Optional<PersistentVolume> getPersistentVolumeByName(KubernetesClient client, String name) {
