	...
	'''
		...
		public class PyroFileController {
			...
			«FOR g:gc.mglModels.map[graphModels].flatten»
				...
				@javax.annotation.security.RolesAllowed("user")
				public Response export«g.name.fuEscapeJava»(@javax.ws.rs.core.Context SecurityContext securityContext, @javax.ws.rs.PathParam("id") final long id,@javax.ws.rs.PathParam("path") final String path) {
			...
			public Response updateFile(@javax.ws.rs.core.Context SecurityContext securityContext, UpdatePyroFile file) {
			    ...
			    «FOR g:gc.graphMopdels + gc.ecores»
			    	{
			    		final Optional<«g.entityFQN»> optG = «g.entityFQN».findByIdOptional(file.getId());
			    		if(optG.isPresent()) {
			    			«g.entityFQN» f = optG.get();
			    			f.filename = file.getfilename();
			    			update = f;
			    			pf = graphModelController.getParent(f);
			    		}
		    ...
		    public Response moveFile(
				...
				«FOR g:gc.graphMopdels + gc.ecores»
					{
					    final «g.entityFQN» file = «g.entityFQN».findById(id);
					    if(file != null) {
					        final PanacheEntity source = graphModelController.getParent(file);
					        if(source instanceof entity.core.PyroFolderDB) {
					            ((entity.core.PyroFolderDB)source).files_«g.name.fuEscapeJava».remove(file);
					        } else if(source instanceof entity.core.PyroProjectDB) {
					            ((entity.core.PyroProjectDB)source).files_«g.name.fuEscapeJava».remove(file);
					        }
					        if(target instanceof entity.core.PyroFolderDB) {
					            if( ((entity.core.PyroFolderDB)target).files_«g.name.fuEscapeJava».stream().filter(n->n.filename.equals(file.filename)).findAny().isPresent()) {
					                return Response.status(Response.Status.BAD_REQUEST).entity("Name already exists").build();
					            }
					            ((entity.core.PyroFolderDB)target).files_«g.name.fuEscapeJava».add(file);
					            file.parent = target;
					        } else if(target instanceof entity.core.PyroProjectDB) {
					            if( ((entity.core.PyroProjectDB)target).files_«g.name.fuEscapeJava».stream().filter(n->n.filename.equals(file.filename)).findAny().isPresent()) {
					                return Response.status(Response.Status.BAD_REQUEST).entity("Name already exists").build();
					            }
					            ((entity.core.PyroProjectDB)target).files_«g.name.fuEscapeJava».add(file);
					            file.parent = target;
					        }
					        source.persist();
					        target.persist();
